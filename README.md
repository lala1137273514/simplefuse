...
- 🔗 **Dify 集成** - 通过 Webhook 自动采集 Dify 工作流的请求与响应
...
2. **配置 LLM** - 进入 设置 > LLM 配置，添加评测用的 LLM（如 OpenAI、Anthropic）
3. **查看 Traces** - 当 Dify 工作流被调用时，会自动采集到 Traces 页面
4. **创建评测器** - 进入 评测器管理，创建评测维度（如相关性、准确性等）
5. **运行评测** - 进入 评测中心，选择 Traces 和评测器，开始评测
6. **查看结果** - 评测完成后自动跳转到结果页，查看各维度评分
