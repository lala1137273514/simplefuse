...
export async function fetchDifyWorkflows(
  ...
  try {
    ...
    return apps.map((app: { id: string; name: string; description?: string }) => ({
      id: app.id,
      name: app.name,
 ...
 * 生成 Webhook URL
 */
export function generateWebhookUrl(connectionId: string, baseUrl?: string): string {
  const base = baseUrl || process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'
  return `${base}/api/webhook/${connectionId}`
}
