...
// Mock Eval Executor
vi.mock("../lib/eval-executor", () => ({
  executeEvaluationBatch: vi.fn(),
}));
...
describe("evalJobs tRPC router", () => {
  ...
  describe("evalJobs.create", () => {
    ...
    it("应该在评测失败时返回具体的错误信息", async () => {
      vi.mocked(prisma.evaluatorTemplate.findMany).mockResolvedValue(
        [mockEvaluators[0]] as any,
      );
      vi.mocked(prisma.evalJob.create).mockResolvedValue(mockJob);

      // Mock executeEvaluationBatch returning failure with error message
  ...
  describe("evalJobs.getById", () => {
    ...
    it("任务不存在时应该抛出错误", async () => {
      ...
      await expect(
        caller.evalJobs.getById({ id: "non-existent" }),
